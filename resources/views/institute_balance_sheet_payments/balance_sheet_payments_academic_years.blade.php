                                ...
                                <table id="datatable_fixed_column" class="table table-condensed table-bordered" width="100%">
                                    ...
                                    <thead> 
                                        <tr>
                                            ...
                                            <th class="hasinput" style="width:10%">
                                                <input type="text"  class="form-control" placeholder="Total Payments" />
                                            </th>
                                             <th class="hasinput" style="width:10%">
                                        ...
                                        <tr>
                                            ...
                                            <th data-sortable="true">Total Payments</th>
                                    ...
                                    <tbody>                                     
                                        ...
                                        foreach ($years as $year) { ?>                                                                           
                                            <tr class="">      
                                                <td>{{$i}}</td>
                                                <td><a href="{{url('balance-sheet-payments-months/'.$year->ac_id)}}">{{$year->start_year}} - {{$year->end_year}}</a></td>
                                                <td><a href="{{url('balance-sheet-payments-months/'.$year->ac_id)}}">{{$year->start_date}} to {{$year->end_date}}</a></td>
                                                 <td>{{$year->start_month}} - {{$year->start_year}} to {{$year->end_month}} - {{$year->end_year}}</td>                                                              
                                                 <td>&#8377; {{$year->total}}</td>
                                            <td>
                                                    <a class="btn btn-xs btn-info" href="{{url('balance-sheet-payments-pdf/2/'.$year->id)}}"><i class="glyphicon glyphicon-file"></i></a>
                                                    <a class="btn btn-xs btn-info" href="{{url('balance-sheet-payments-pdf/1/'.$year->id)}}" target="_blank"><i class="glyphicon glyphicon-print"></i></a>
                                                </td>
                                            </tr>
    <?php $i++;
} ?>
