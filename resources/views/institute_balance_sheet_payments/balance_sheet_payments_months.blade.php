                ...
                <div class="col-xs-12 col-sm-12 col-md-12 col-lg-9">
                    <div class="jarviswidget " id="wid-id-3" data-widget-editbutton="false">
                        <header>
                            ...
                            <a type="button" class="btn bg-color-blueLight txt-color-white btn-xs pull-right" href="{{url('balance-sheet-payments-academic-years')}}" style="margin-top: 5px;margin-right: 5px;"><i class="fa fa-arrow-left"></i>  BACK</a>
                        </header>
                        </header>
                        <div>
                                    ...
                                    <table id="datatable_fixed_column" class="table table-condensed table-bordered" width="100%">
                                        ...
                                        <tbody>                                     
                                            ...
                                            foreach ($months as $month) {
                                                ...
                                                <tr class="">      
                                                    <td>{{$i}}</td>
                                                    <td><a href="{{url('balance-sheet-payments-day/'.$years[0]->ac_id.'/'.$month->year.'/'.$month->month)}}">{{$month->month}} - {{$month->year}}</a></td>
                                            ...
                                            ?>
                                            @if($months !='')      <tr><td><i>Academic Year : {{$years[0]->start_year}} - {{$years[0]->end_year}}</i></td><td>Total Payments</td><td><i> &#8377; {{ $total }} </i></td></tr>@endif
                                        </tbody>
                                    </table>
...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        ...
        $.ajax({
            type: 'GET',
            url: '/balance-sheet-payments-chart',
            dataType: 'json',
            success: function (data) {
                var p_labels = data.labels;
                var payment_data = data.p_data;
