                ...
                <div class="col-xs-12 col-sm-12 col-md-12 col-lg-9">
                    <div class="jarviswidget " id="wid-id-3" data-widget-editbutton="false">
                        <header>
                            ...
                            <h2>View Expenses of @if($days !=0)<b> {{$days[0]->month_name}} - {{$days[0]->year}} : {{$each_day[0]->date}} ({{ $each_day[0]->day }})</b> @endif</h2>
                           
                            <a type="button" class="btn bg-color-blueLight txt-color-white btn-xs pull-right" href="{{url('balance-sheet-expenses-day/'.$years[0]->ac_id.'/'.$days[0]->year.'/'.$days[0]->month_name)}}" style="margin-top: 5px;margin-right: 5px;"><i class="fa fa-arrow-left"></i>  BACK</a>
                        </header>
                        </header>
                        <div>
                                    ...
                                    <table id="datatable_fixed_column" class="table table-condensed table-bordered" width="100%">
                                        ...
                                        <tbody>                                     
                                            ...
                                            foreach ($each_day as $each) {
                                                 if( $each_day != '')  {  $total +=$each->total_expenses; }
                                                ?>                                                                           
                                                <tr class="">      
                                                    <td>{{$i}}</td>
                                                    <td>{{$each->date}}({{$each->day}})</td>
                                                    <td><a href="{{url('balance-sheet-expense-history/'.$years[0]->ac_id.'/'.$days[0]->year.'/'.$days[0]->month_name.'/'.$each->today.'/'.$each->expense_type_id)}}">{{$each->title}}</a></td>
